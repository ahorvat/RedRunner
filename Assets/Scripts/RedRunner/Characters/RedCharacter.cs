...
namespace RedRunner.Characters
{
    ...
    public class RedCharacter : Character
    {
        ...
        void Update()
        {
            ...
            if(CrossPlatformInputManager.GetButtonDown("Save"))
            {
                if(!IsDead.Value)
                {
                    ...
                    {
                        if(GameManager.Singleton.m_Coin.Value >= 5)
                        {
                            GameManager.Singleton.m_Coin.Value -= 5;
                            GameManager.Singleton.SaveState = new SaveState(m_Rigidbody2D.position);
                            Debug.Log($"Savestate created: {GameManager.Singleton.SaveState.Position.ToString()}");
                        }
                        else
                        {
                            Debug.LogWarning("Not enough coins to create save state!");
                        }
                    }
                }
            }
        ...
        void GameManager_OnReset()
        {
            ...
            {
                transform.position = new Vector3(GameManager.Singleton.SaveState.Position.x, GameManager.Singleton.SaveState.Position.y);
                GameManager.Singleton.m_QuickLoading = false;
                Debug.Log("Finished Quickloading");
            }
            else
            {
                transform.position = m_InitialPosition;
                Debug.Log("Finished Slowloading");
            }
            Reset();
        }
